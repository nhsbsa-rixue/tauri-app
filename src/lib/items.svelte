...
<div class="utility-card h-full flex flex-col">
  <div class="flex items-center justify-between mb-4">
    <h2 class="typography-tagline flex items-center gap-2">
    ...
    {#if $items.length > 0}
      <button
        type="button"
        class="btn-dark-utility typography-button-utility"
        onclick={() => clearItems()}
      >
        <Trash2 size="14" />
        <span>Clear</span>
  ...
  <div class="flex-1 overflow-y-auto">
    {#if $items.length === 0}
      <div
        class="flex flex-col items-center justify-center h-full opacity-50 gap-2 typography-caption"
      >
    ...
    {:else}
      {#each $items as item, i}
        <div class="list-row">
          <span class="typography-body truncate flex-1">
  ...
  {#if $items.length > 0}
    ...
    >
      <div class="flex justify-between typography-body-strong mb-4">
        <span class="typography-body-strong">Total</span>
        <span>${total.toFixed(2)}</span>
      </div>
      <button
        type="button"
        class="btn-primary-pill typography-body w-full"
